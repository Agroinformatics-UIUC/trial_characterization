...
if(!(file.exists(file.path(.libPaths(), "apsimr", 'DESCRIPTION')) & packageVersion("apsimr") == '1.2')){
  install.packages(paste0(codes_folder, "/R_packages_not_in_CRAN/apsimr_1.2.tar.gz"), repos = NULL, type = "source")
} 
if(!(file.exists(file.path(.libPaths(), "lmeInfo", 'DESCRIPTION')) & packageVersion("lmeInfo") == '0.2.1')){
  install.packages(paste0(codes_folder, "/R_packages_not_in_CRAN/lmeInfo_0.2.1.tar.gz"), repos = NULL, type = "source")
}
if(!(file.exists(file.path(.libPaths(), "soilDB", 'DESCRIPTION')) & packageVersion("soilDB") == '2.6.14')){
...
if(length(new.packages) != 0) {lapply(new.packages, function(x) {install.packages(x, dependencies=TRUE)}, character.only = TRUE)}
