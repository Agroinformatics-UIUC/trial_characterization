...
expand_table <- function(x,y){
  ...
  x_nrow <- nrow(x)
  z <- cbind(x[rep(1:x_nrow, each = nrow(y)), ], y)
  return(z)
}
...
eonr_from_rf <- function(pred_set, trial_rates, rf_model){
  ...
  pred_set2_list <- split(pred_set2, by=by_c)
  
  eonr_rates <- seq(min(trial_rates),max(trial_rates), by = 1)
  
  best_rate <- list()
  for(i in 1:length(pred_set2_list)){
    pred_set2_small <- pred_set2_list[[i]]
    model_loess <- loess(yld_pred ~ N_fert, span = 0.5, data=pred_set2_small)
    numeric_prediction <-data.table(unique(pred_set2_small[,c(by_c), with = FALSE]), 
               N_fert = eonr_rates)
    numeric_prediction$yld_pred <- stats::predict(model_loess, numeric_prediction)
    
    ggplot() +
      geom_point(data = numeric_prediction, aes(x = N_fert, y = yld_pred)) +
      geom_point(data = pred_set2_small, aes(x = N_fert, y = yld_pred, color = 'red', size = 2))
    
    numeric_prediction[, P := yld_pred * Pc - N_fert * Pn]
    pred_set2_small[, P := yld_pred * Pc - N_fert * Pn]
    
    ggplot() +
      geom_point(data = numeric_prediction, aes(x = N_fert, y = P)) +
      geom_point(data = pred_set2_small, aes(x = N_fert, y = P, color = 'red', size = 2))
    
    best_rate[[length(best_rate)+1] <- numeric_prediction[P == max(P)][1]
    
  }
  
  reg_models_list <- lapply(pred_set2_list, function (x) loess(yld_pred ~ N_fert, span = 0.5, data=x))
  
  #3) Numerically find EONR
  
  numeric_prediction <- expand_table(names(reg_models_list), eonr_rates)
  setnames(numeric_prediction, c('x', 'y'), c('model', 'N_fert'))
  
  
  numeric_prediction_list <- split(numeric_prediction, by= 'model' )
  lapply(numeric_prediction_list, function(x) reg_models_list[[unique(x$model)]])
  
  numeric_prediction[,yld_pred := reg_models_list[[model]]]
  stats::predict(reg_models_list[['1592712.A16.0']], 60)
  ...
  ggplot() +
    geom_point(data = pred_set3[mukey == 1592712 & z == 'A16' & rotation == 0], aes(x = N_fert, y = Yld_pred)) +
    geom_smooth(data = pred_set2[mukey == 1592712 & z == 'A16' & rotation == 0], aes(x = N_fert, y = yld_pred, color = 'red', size = 2))
